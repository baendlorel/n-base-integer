...
const removeConsole: TransformerFactory<keyof ts.CustomTransformers> = {
  type: 'program',
...
const privatify = {
  type: 'program',
  factory: (program: ts.Program) => {
    function findPrivateMembers(sourceFile: ts.SourceFile): ts.Node[] {
      const results: ts.Node[] = [];
      const visit = function (node: ts.Node): void {
        // 检查属性或方法声明
        if (
    ...
    return (context: ts.TransformationContext) => {
      return (sourceFile: ts.SourceFile) => {
        const privates = findPrivateMembers(sourceFile);
        console.log('privates', privates);
        return sourceFile; // 保持 AST 不变
      };
    };
  },
};
